...
@section('page_title')
    {{ isset($auction) ? 'Edit Challenge Mechanism' : 'Add Challenge Mechanism' }}
@endsection
    ...
    <div class="card form-card">
            ...
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="text-white font-weight-bold mb-0">
                        {{ isset($auction) ? 'Edit Challenge Mechanism' : 'Add New Challenge Mechanism' }}</h4>
                    <small
                        class="text-white-50">{{ isset($auction) ? 'Update challenge mechanism details' : 'Fill in the details to create a new challenge mechanism' }}</small>
                </div>
            </div>
        ...
        <form action="{{ isset($auction) ? route('auctions.update', $auction) : route('auctions.store') }}" method="POST">
            ...
            <div class="card-body px-4 py-4">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrap">
                                ...
                                <input type="text" name="base_price"
                                    ...
                                    value="{{ old('base_price', $auction->base_price ?? '') }}" placeholder="e.g. 5000000">
                            </div>
                            @error('base_price')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-4 col-12">
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrap">
                                ...
                                <select name="increment_amount_type"
                                    class="form-control @error('increment_amount_type') is-invalid @enderror">
                                    <option value="">-- Select Type --</option>
                                    <option value="recommend"
                                        {{ old('increment_amount_type', $auction->increment_amount_type ?? '') == 'recommend' ? 'selected' : '' }}>
                                        Recommended</option>
                                    <option value="mandatory"
                                        {{ old('increment_amount_type', $auction->increment_amount_type ?? '') == 'mandatory' ? 'selected' : '' }}>
                                        Mandatory</option>
                                </select>
                            </div>
                            @error('increment_amount_type')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            @unless ($errors->has('increment_amount_type'))
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon-wrap">
                                ...
                                <select name="increment_type"
                                    class="form-control @error('increment_type') is-invalid @enderror">
                                    <option value="">-- Select Type --</option>
                                    <option value="fixed"
                                        {{ old('increment_type', $auction->increment_type ?? '') == 'fixed' ? 'selected' : '' }}>
                                        Any Value</option>
                                    <option value="multiple"
                                        {{ old('increment_type', $auction->increment_type ?? '') == 'multiple' ? 'selected' : '' }}>
                                        Multiple</option>
                                </select>
                            </div>
                ...
                {{-- Resolution Professional (admin only) --}}
                @role('admin')
                <p class="form-section-title mt-3"><i class="fas fa-user-tie mr-1"></i> Resolution Professional <span class="text-danger">*</span></p>
                <div class="row">
                    <div class="col-md-6 col-12">
                        <div class="form-group">
                            @if($rpUsers->count())
                                <div class="input-icon-wrap">
                                    ...
                                    <select name="rp_user_id" class="form-control @error('rp_user_id') is-invalid @enderror">
                                        <option value="">-- Select Resolution Professional --</option>
                                        @foreach($rpUsers as $rp)
                                            <option value="{{ $rp->id }}"
                                                {{ old('rp_user_id', $auction->created_by ?? '') == $rp->id ? 'selected' : '' }}>
                                                {{ $rp->name }} — {{ $rp->email }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('rp_user_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                            @else
                                <div class="alert alert-warning mb-0" style="border-radius:10px;">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>
                                    No users with <strong>RP</strong> role found. Please assign the RP role to users first.
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                {{-- Participants --}}
                <p class="form-section-title mt-3"><i class="fas fa-users mr-1"></i> Resolution Applicant <small
                        class="text-muted font-weight-normal text-lowercase">(Resolution Applicante users)</small></p>
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                            @if ($participants->count())
                                @php
                                    $selectedParticipants = old(
                                        'participants',
                                        isset($auction) ? $auction->participants->pluck('user_id')->toArray() : [],
                                    );
                                @endphp
                                <select id="participants" name="participants[]" multiple>
                                    @foreach ($participants as $p)
                                        <option value="{{ $p->id }}"
                                            {{ in_array($p->id, $selectedParticipants) ? 'selected' : '' }}>
                                            {{ $p->name }} — {{ $p->email }}
                                        </option>
                                    @endforeach
                                </select>
                            @else
                                <div class="alert alert-warning mb-0" style="border-radius:10px;">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>
                                    No users with <strong>Resolution Applicant</strong> role found. Please assign the
                                    Resolution Applicant role to users first.
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                {{-- NPV Categories --}}
                <p class="form-section-title mt-3"><i class="fas fa-tags mr-1"></i> Description of Creditors <span
                        ...
                        <div class="form-group">
                            @php
                                $selectedCategories = old(
                                    'npv_categories',
                                    isset($auction) ? $auction->npvCategories->pluck('id')->toArray() : [],
                                );
                            @endphp
                ...
                @php
                    $npvpRows = old('npvp');
                    if ($npvpRows === null) {
                        $npvpRows = isset($auction)
                            ? $auction->npvpConfigurations
                                ->map(
                                    fn($n) => [
                                        'period' => $n->period,
                ...
                <div id="npvp-wrapper">
                    @foreach ($npvpRows as $i => $npvp)
                        <div class="npvp-row" id="npvp-row-{{ $i }}">
                            <button type="button" class="btn btn-danger btn-sm btn-remove-npvp"
                                onclick="removeNpvp({{ $i }})">
                            ...
                            <div class="row">
                                <div class="col-md-6 col-12">
                                    <div class="form-group mb-2">
                                        ...
                                        @error('npvp.' . $i . '.period')
                                            <div class="npvp-error text-danger" style="font-size:12px;">{{ $message }}
                                            </div>
                                        @else
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="form-group mb-2">
                                        <label class="mb-1">Discounting Factor <span
                                                class="text-danger">*</span></label>
                                        <input type="number" step="0.0000001"
                                            name="npvp[{{ $i }}][percentage_value]"
                                        ...
                                        @error('npvp.' . $i . '.percentage_value')
                                            <div class="npvp-pct-error text-danger" style="font-size:12px;">
                                                {{ $message }}</div>
                                        @else
                                            <div class="npvp-pct-error text-danger" style="font-size:12px; display:none;">
                                            </div>
                                        @enderror
                                    </div>
                                </div>
            ...
            <div class="card-footer bg-white border-top px-4 py-3 d-flex flex-wrap align-items-center"
                style="border-radius:0 0 16px 16px;">
                <button type="submit" class="btn btn-submit">
                    <i
                        class="fas fa-{{ isset($auction) ? 'sync-alt' : 'save' }} mr-2"></i>{{ isset($auction) ? 'Update Challenge Mechanism' : 'Create Challenge Mechanism' }}
                </button>
